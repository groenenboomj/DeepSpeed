                ...
                for n, p in module.named_parameters():
                    if prefix + n in sd[0] and len(n.split('.')) == 1:
                        if type(sd[0][prefix + n]) is list:
                            ...
                            weight_quantizer.num_groups = scale.shape[0] #New
                        else:
                        ...
                        if (len(src_shape) == 2 and len(dst_shape) == 2):
                            if (src_shape[inner_dim] == dst_shape[0] and src_shape[outer_dim] == dst_shape[1]):
                                if tmp_data.dtype != torch.int8:
                                    ...
                                    #p = torch.nn.parameter.Parameter(tmp_data, requires_grad=False) #NEW
                                    #p.scale = scale
                                ...
                                if tmp_data.dtype != torch.int8:
                                    ...
                                    weight_partition = weight_quantizer.quantize(torch.nn.parameter.Parameter(
                                        weight_partition,
                                        requires_grad=False), scale=scale)
                                    #weight_partition = torch.nn.parameter.Parameter(weight_partition, requires_grad=False) #NEW
